...
stripes; the top-edge fade runs on top. `flat` / `foot-y` / `tile-y` /
`foot-smooth` / `tile-smooth` / `off` remain available via `setObjectLightingMode()`.
...
## 9. Runtime tuning — console commands

All the lighting/rendering knobs added during the §6–§8 work are live-switchable
from the browser DevTools console (each persists in `Config.engine.*` and takes
effect next frame). Player-light smoothing is detailed in
[`wiki/lighting.md`](lighting.md) → "Moving-light smoothing".
...
## 10. Summary — agreements, deviations, approximations
