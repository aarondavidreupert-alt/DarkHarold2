...
- **Bit 30 substituted for bit 31 in the occlusion branch (Task 2), severity:
  ...
  **Update**: `isCEOccludingWall()` itself is left untouched, but a new sibling
  ...
  mode, `setEggMode('ce-egg')`, using the same `egg.png` mask rendering
  path as `'dh2-egg'` mode so the two are visually A/B-comparable in-browser.
  (Both modes were renamed from `'egg'`/`'ce-literal'` to `'dh2-egg'`/`'ce-egg'`
  on 2026-06-30 for clarity now that a third "naive" comparison concept exists
  in the lighting system too — see `wiki/known_bugs.md` RD16 addendum.)
