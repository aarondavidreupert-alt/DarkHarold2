...
| U4 | **HUD reload AP hardcoded to 2 — mostly correct.** VERIFIED 2026-06-02: CE `item.cc:1650 weaponGetActionPointCost` returns 2 for all reloads unless weapon has `PERK_WEAPON_FAST_RELOAD` (→1) or is Solar Scorcher (→0). Both exceptions require reading weapon proto perk field which is not loaded in DH2. Hardcode is correct for all standard weapons. | `ui_hud.ts:195`, `ui.ts:323` | `item.cc:1643 weaponGetActionPointCost` | minor | partial |
