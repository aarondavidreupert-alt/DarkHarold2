...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| AC1 | **Knockback now implemented.** FIXED 2026-06-02 — `critterDamage()` computes `floor(damage/10)` tiles (CE ref: `combat.cc:4651 knockbackDistanceDivisor=10`) and slides the defender away from the attacker (direction = `hexDirectionTo(attacker, defender)`), stopping at blocking objects. Respects `CRITTER_NO_KNOCKBACK=0x4000` proto flag. Outstanding: no Stonewall perk check (50% halve), no PERK_WEAPON_KNOCKBACK divisor-5 path. | `src/critter.ts` | `actions.cc:102 actionKnockdown`, `combat.cc:4651` | major | partial |
