...
| RD16 | **Egg transparency implemented.** FIXED 2026-06-15, refined 2026-06-17 — Walls and scenery in front of the player become transparent so the player is visible behind them. Two modes: `'alpha'` (flat α, simpler) and `'egg'` (CE-faithful per-pixel mask via `egg.png`, **now the default**). Condition matches CE default case: `hexIsToRightOf(player, obj)` + `hexIsInFrontOf` within configurable hex radius (`setEggRadius()`). 2026-06-17 fixes: (1) egg anchor now derives from the player's actual per-frame render position (`objectRenderInfo`) instead of raw hex position + a CE-style `+16/+8` corner-to-center fudge — DH2's `hexToScreen()` already returns a bottom-center/foot anchor (unlike CE's `tileToScreenXY` corner), so the extra fudge was shifting the egg one half-hex NW; using the actual render anchor also makes the egg track walk-animation shift smoothly frame-to-frame. (2) In `'egg'` mode the wall now defaults to fully opaque outside the small egg-texture footprint (matching CE — no separate flat-alpha fallback layer) instead of blending with `u_alpha`, which previously made every qualifying wall look tinted everywhere with just a hole cut in the center. Known remaining limitation: the egg's visual footprint is capped at the actual `egg.png` size (129×98 px, ~2-3 hex tiles), same as CE's `egg.frm` — immediately-adjacent walls falling in the transparent PNG corner outside the oval may show no visible fade even though they qualify; this mirrors original CE/jsFO behavior rather than being a DH2-specific bug. Togglable via `Config.ui.showEgg`, tunable via `setEggMode()`/`setEggAlpha()`/`setEggRadius()`/`debugEgg()` console commands. CE ref: `object.cc:4949 _obj_render()`. | `src/render/webglDraw.ts`, `shaders/fragment.glsl`, `src/render/webglContext.ts`, `src/config.ts` | `object.cc:4949 _obj_render()`; `tile.cc tileIsToRightOf()` | major | fixed |
