...
| CI8 | **`target_highlight` 3-state.** FIXED 2026-06-04 — `Config.ui.targetHighlight` retyped as `'off' \| 'on' \| 'targeting-only'`; UI cycle button shows the three labels; `combat.ts` outline gate respects targeting-only (only highlights when `cursorMode === 'attack'`). Legacy boolean preferences migrated to string at load time. CE ref: `game_config.h:111 TargetHighlight`. | `src/config.ts:23`, `src/combat.ts:1592`, `src/ui_options.ts:232` | `game_config.h:111` | low | fixed |
...
| LE7 | **`item_caps_total` live-sums money piles.** FIXED 2026-06-04 — now iterates inventory for `pid === MONEY_PID` and sums amounts; `obj.money` getter also updated to sum (was returning only the first stack). CE ref: `item.cc item_caps_total`. | `src/scripting.ts:837`, `src/object.ts:728` | `item.cc item_caps_total()` | minor | fixed |
...
| LE11 | **Multi-pile caps now sum.** FIXED 2026-06-04 — `Obj.money` getter sums all `MONEY_PID` inventory entries; previously returned only the first stack. Tile pickup still passes `this.amount` per item which is correct for the loose-item model. CE ref: `proto_instance.cc:571 _obj_pickup()`. | `src/object.ts:728` | `proto_instance.cc:571 _obj_pickup()` | low | fixed |
