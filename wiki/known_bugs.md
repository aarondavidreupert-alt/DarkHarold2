...
| FA12 | **Per-orientation pixel drift on weapon draw — critter shifts 1–2 px per draw cycle for most skins/directions.** FIXED 2026-06-03 — Root cause: `clearAnim`'s zero-jump formula carried `prevArtOffset` forward and the `floor(w/2)` correction is non-zero whenever the draw FRM is not left-edge-consistent (`floor(wN/2) − oxN ≠ floor(w0/2) − ox0`). This gives `K_draw = floor(dW0/2) − floor(dWN/2) + dOxN − dOx0` per full animation cycle; 201/242 tested skin bases had `K_draw ≠ 0` for E (dir1). Fix: `Critter.clearAnim` now unconditionally resets `artOffset = {0,0}` instead of applying the formula, matching CE's `objectSetLocation` which zeroes `obj->x/y` on every tile change. The worst-case snap at animation-end equals `|K_net|` (0–2 px for well-designed FRMs), which is imperceptible compared to multi-pixel cumulative drift causing tile jumps. | `src/object.ts Critter.clearAnim` | `object.cc objectSetLocation`; `animation.cc artGetFrameOffsets` | minor | **FIXED 2026-06-03** |
