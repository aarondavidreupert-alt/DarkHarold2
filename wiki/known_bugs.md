...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| IW1 | **Indicator bar wired.** FIXED 2026-06-04, updated 2026-06-11 (6-slot fix) — `updateIndicatorBar()` renders exactly 6 fixed-width slots (CE INDICATOR_SLOTS_COUNT=6) in order ADDICT/SNEAK/LEVEL/POISONED/RADIATED+empty; empty slots are transparent spacers preserving layout; bar is parented inside `#bar` at `bottom:100%` (replaces fragile offsetHeight calculation). CE ref: `interface.cc indicatorBarRefresh`. | `src/ui_hud.ts` | `interface.cc indicatorBarInit()` | major | fixed |
