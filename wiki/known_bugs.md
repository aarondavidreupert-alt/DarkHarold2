...
| C13 | **SNIPE distance mode now implemented.** PARTIAL 2026-06-02 — `doAITurn()` now handles `snipe`: if distance < 10, back away from target along the opposite direction until 10 tiles or AP exhausted. CE ref: `combat_ai.cc:2999 _cai_perform_distance_prefs DISTANCE_SNIPE`. `DISTANCE_CHARGE` was already the default; `DISTANCE_STAY_CLOSE` (stay within 5 of player) not yet wired (not in DH2's DistanceMode type). | `combat.ts` | `combat_ai.cc:_cai_perform_distance_prefs` | minor | partial |
...
| S2 | `metarule3` IMPROVED 2026-06-02 | `scripting.ts:621` | Sub-ops handled: 100 (clr_fixed_events), 103 (get_kill_count — new, uses critter.ts killCounts map), 106 (tile_get_next_critter), 108 (tile_set_center — new, calls centerCamera). IDs 101/102/104/105/107/109-111 still stub. | minor | partial |
| S3 | `get_critter_stat` | `scripting.ts:608` | **FIXED 2026-06-02** — Sequence (stat 13) now calls `obj.getStat('Sequence')` = `2*PER` (was wrong `10+2*PER`). 26+ stat IDs handled. STAT_AGE (33) still returns stub 5. | major | partial |
...
| S5 | `critter_add_trait` | `scripting.ts:672` | **FIXED 2026-06-02** — Cases 5 (ai\_packet), 6 (team\_num), 10 (OBJECT\_CUR\_ROT → `obj.orientation`), 666 (OBJECT\_VISIBILITY → `obj.visible`), 669 (OBJECT\_CUR\_WEIGHT → read-only, no-op) implemented. Stub fires only for non-`TRAIT_OBJECT` or unrecognised trait IDs. CE ref: `interpreter_extra.cc opCritterAddTrait`. | minor | partial |
...
| S11 | `anim` | `scripting.ts:1338` | **IMPROVED 2026-06-02** — IDs 1000 (set rotation, clamped 0–5 per CE guard), 1010 (set frame), and anim type IDs 0–64 (wrapped in one-shot reg_anim batch) all handled. CE ref: `interpreter_extra.cc:3355 opAnim`. Stub fires only for out-of-range IDs. Reverse-anim path (param≠0) plays forward due to `singleAnimation` not distinguishing direction. | major | partial |
...
| FA12 | **Per-orientation pixel drift on weapon draw — four-part fix.** FIXED 2026-06-04 — **Part 1 (mid-idle contamination, 2026-06-03):** `staticAnimation` baked `iOxF` into `artOffset_draw`. Fix: `srcF = oldFrames[0]` when `this.anim === 'idle'`. CE ref: `art.cc artGetFrameOffsets`. **Part 2 (K_cycle accumulation, 2026-06-03):** clearAnim zero-jump with `prev` caused −2px/cycle drift for dir2 of hmjmps (K_cycle ≠ 0). Interim fix: `artOffset = {0,0}` hard reset, bounded drift at the cost of a residual Y snap at settle. **Part 3 (Y height correction + clearAnim formula, 2026-06-04):** both `staticAnimation` and `clearAnim` were missing the `(newF0.h − srcF.h)` height term. Confirmed by log: hmjmpsia h=65 → hmjmpsid h=63 → artOffset.y was 4 instead of 2 → 2 px Y jump. Fix: height term added; `clearAnim` uses full zero-jump formula with current artOffset as `prev`. **Part 4 (series-max width for X, 2026-06-04):** X half-width term used per-frame `f0.w`, breaking telescoping when frame-0 and last-frame widths differ within an animation. Fix: `staticAnimation`/`clearAnim` now use `info.frameWidth` (series max = uniform atlas slot) in the artOffset formula; renderer X anchor change reverted (see FA13 — these two are now inconsistent). CE ref: `art.cc artGetWidth(art, frame, rotation)`. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts objectRenderInfo` | `art.cc artGetWidth, artGetFrameOffsets`; `object.cc objectSetLocation` | minor | **FIXED 2026-06-04** |
| FA13 | **artOffset formula and renderer use inconsistent width terms after Part 4 renderer revert.** `staticAnimation`/`clearAnim` (object.ts) compute artOffset using `info.frameWidth` (series-max = atlas-packing uniform slot width, a `frmpixels.py` artifact). `renderer.ts` uses `frameInfo.w/2` (per-frame, restored by the Part 4 revert). The two are only consistent when all frames in a given FRM have identical width. For variable-width FRMs every art transition introduces a horizontal jump equal to `floor(maxW/2) − floor(frameW/2)`. CE has no series-max width concept for art rendering; CE always uses per-frame `artGetWidth(art, frame, rotation)` (`art.cc:735`), and the renderer applies it as `tileScreenX -= width/2` (`object.cc:2347`). CE does not have `artGetFrameWidth`. The `info.frameWidth` field in imageMap.json is purely a pipeline packing artifact. CE-faithful fix: change artOffset formula to also use per-frame width (matching renderer). The Part 4 renderer change (series-max) was reverted because `hmjmpsia` maxW=90 but typical frames ~30px caused a 30px left-shift; at that time the formula was not simultaneously reverted. See `wiki/failed_animation_offset_attempts.md §Hypothesis: Pipeline-level fix` for full CE source verification. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts` | `art.cc artGetWidth(art, frame, rotation)`; `object.cc:2347` | minor | bug |

<!-- audited: 2026-06-04; FA13 added 2026-06-04 -->
