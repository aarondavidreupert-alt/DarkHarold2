...
| CI7 | **`item_highlight` setting wired.** FIXED 2026-06-04 — `Config.ui.itemHighlight` toggled by `controls.highlightItems` (default: Spacebar) via heart keydown/keyup; `renderObjects` paints a yellow outline on `item` objects while held. CE outlines all items when the key is down. CE ref: `game_config.h:37`. | `src/config.ts`, `src/input.ts`, `src/renderer.ts:378` | `game_config.h:37`; `settings.h:33` | low | fixed |
