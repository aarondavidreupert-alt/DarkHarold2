...
| FA7 | **FRM art transition caused sprite position jump.** FIXED 2026-06-03 — three sources: (1) directionOffset mismatch between FRMs (e.g. idle y=5 → draw y=0 = 5 px y jump); (2) non-zero per-frame ox at transition frame (idle dir0 frames 2–10 have ox=1; ignoring this = 2 px x jump); (3) frame-width center-anchor change (dir4: idle w=23 → draw w=34 = 6 px anchor shift, total -12 px). Fix: `artOffset: Point` field on `Obj`, updated at every art switch using exact zero-jump formula: `artOffset.x = floor(newW/2)−floor(oldW/2) + oldDirOff.x−newDirOff.x + oldOx[F]−newOx[0]`, `artOffset.y = oldDirOff.y−newDirOff.y + oldOy[F]−newOy[0]`. Computed synchronously before `lazyLoadImage` (imageInfo always available). Resets to 0 on walk end (matches CE's `objectSetLocation` reset). Full model in `wiki/animation.md §4.2`. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts` | `animation.cc ~2886 artGetRotationOffsets + artGetFrameOffsets`; `object.cc objectSetLocation` | medium | **FIXED 2026-06-03** |
...
| FA12 | **Per-orientation pixel drift on weapon draw — two-part fix.** FIXED 2026-06-03 — **Part 1 (mid-idle contamination):** `staticAnimation`'s zero-jump formula baked `iOxF` (idle ox at trigger frame F) into `artOffset_draw`, displacing the critter throughout the holster animation depending on which idle frame draw was triggered from. Fix: when `this.anim === 'idle'`, use `srcF = oldFrames[0]` (frame 0 geometry anchor) so `artOffset` is independent of which idle frame was active. CE ref: `art.cc artGetFrameOffsets`. **Part 2 (K_cycle accumulation):** even with the anchor fix, `clearAnim` carried `artOffset` forward via the zero-jump formula's `prev` term. For some FRM directions (e.g. dir2 of hmjmps = K_cycle = −2px/swap) the FRM set is not a perfectly closed loop, so `prev` grew unboundedly (−2px/cycle for dir2). Fix: `clearAnim` now resets `artOffset = {0,0}` unconditionally, mirroring CE's `objectSetLocation` (object.cc) which resets `obj->x/y` to tile screen coords on every settle. `staticAnimation`'s zero-jump formula (using the frame-0 anchor) still ensures visual continuity at the START of each animation; the 0–1px snap at the END (last-frame → idle f0) is the same snap CE has due to FRM rounding. | `src/object.ts Critter.staticAnimation, clearAnim` | `art.cc artGetFrameOffsets`; `object.cc objectSetLocation` | minor | **FIXED 2026-06-03** |
