...
| FA13 | **`artOffset` mechanism removed — DarkFO direct-swap model.** SUPERSEDED 2026-06-06 — Empirical testing of the FA7/FA12 carry formula revealed that for FRM sets with `K_cycle ≠ 0` (e.g. hmjmps `i↔k` weapon-swap chain leaks +4 px per cycle), the carry accumulates unbounded drift between walks. Resetting the carry to `{0,0}` on `clearAnim` bounds the drift but reintroduces a visible per-settle snap. No single carry strategy gives both zero per-transition jump and zero accumulated drift when the underlying FRM data is asymmetric. Decision: remove `artOffset` field, `staticAnimation`/`clearAnim` carry formula, and renderer `+ obj.artOffset` term. Sprite is now positioned from each frame's own `ox`/`oy`/`dirOff` directly (DarkFO behaviour). Trade-off: small per-transition jumps remain for dir1/dir4 of some critters (0–12 px depending on FRM set), zero drift over arbitrary swap sequences. Full forensic record: `wiki/failed_animation_offset_attempts.md`. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts objectRenderInfo, objectBoundingBox` | DarkFO `darkfo/src/object.ts` | minor | **SUPERSEDED 2026-06-06** |
