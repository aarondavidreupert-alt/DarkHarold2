...
| C10 | **Unarmed special moves applied in combat.** VERIFIED 2026-06-04 — `combat.ts:438` adds `mode.critBonus` to crit chance; `combat.ts:670 getUnarmedDamageDone` reads `mode.minDmg/maxDmg` for damage roll and `mode.penetrate` for the DT-to-20% bypass. Active mode selected via `getActiveUnarmedModeForHand` (player) or `getActiveUnarmedMode(skill, 0)` (AI fallback). AI hardcodes 3 AP, which matches mode 0 'punch' so no functional gap. Wiki entry was stale. | `combat.ts:438,670`, `unarmed.ts` | `unarmed.cc` | minor | fixed |
...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| U1 | **Preferences screen implemented.** VERIFIED 2026-06-04 — `openPrefsPanel()` (`ui_options.ts:335`) renders a 434-line panel covering difficulty, combat speed, violence level, run-by-default, subtitles, and master/music/sfx volume sliders. Bound to the `P` shortcut and the Options button; settings persist to localStorage via `loadPreferences`. Wiki entry was stale. | `ui_options.ts:335` | `preferences.cc` | major | fixed |
...
| FA4 | **Combat walk-speed boost wired.** FIXED 2026-06-04 — `Critter.updateWalkAnim` adds `Config.combat.combatSpeed` to walk/run `fps` while `globalState.inCombat`. Player-speedup pref is moot in DH2 so the boost always applies during combat. CE ref: `animation.cc:3287 animationComputeTicksPerFrame`. | `object.ts:1538` | `animation.cc:3287` | minor | fixed |
...
| LE9 | **Container loot fires `use_p_proc`.** FIXED 2026-06-04 — `main.ts showTargetInventory` now runs `Scripting.use(container, player)` first; only opens loot if the script did not call `script_overrides`. CE ref: `proto_instance.cc _obj_use_container`. Dead-critter loot path retains direct `uiLoot` (corpses have no use proc). | `src/main.ts:879` | `proto_instance.cc _obj_use_container()` | minor | fixed |
