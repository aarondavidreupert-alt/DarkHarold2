...
| FA7 | **FRM art transition caused sprite position jump (directionOffsets discontinuity).** FIXED 2026-06-03 (rev 2) — switching FRMs (e.g. idle `hanpwraa` with `directionOffsets[0].y=5` → weapon-draw `hanpwrjc` with `directionOffsets[0].y=0`) immediately changed the renderer's `dirOffset` with no carry-over, causing a 5 px visible Y jump. CE prevents this via `_obj_offset(OLD_art->xOffsets + newFrame0.rawDelta, ...)` at transition (`animation.cc ~2886`), folding the old header offsets into `obj->x/y`. DH2 fix: added `artOffset: Point` to `Obj`; at every art switch, formula `artOffset = oldDirOff + oldArtOffset − newDirOff` is applied (no per-frame ox terms — those are handled by the renderer's `frameInfo.ox`). Computed synchronously from `globalState.imageInfo` **before** `lazyLoadImage` so position is correct on the first render frame even on a cold texture cache. `artOffset` resets to `{0,0}` on walk end (matches CE's `objectSetLocation` reset). Full model documented in `wiki/animation.md §4.2`. | `src/object.ts Critter.staticAnimation, Critter.clearAnim`; `src/renderer.ts` | `animation.cc ~2886 _obj_offset()`; `art.cc artGetRotationOffsets()`; `object.cc objectSetLocation` | medium | **FIXED 2026-06-03** |
