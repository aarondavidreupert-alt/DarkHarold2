...
| FA12 | **Per-orientation pixel drift on weapon draw.** FIXED 2026-06-03 (revised) — Root cause: `staticAnimation` bakes `iOxF` (idle ox at trigger frame F) into `artOffset_draw` for visual continuity. `clearAnim`'s zero-jump formula then carries that term back into the new idle artOffset, giving `K_per_cycle = K_draw + iOxF` where `iOxF` is random (0–6 px, depending on which idle frame draw was triggered from). Fix: at idle→oneshot in `staticAnimation`, compute `_idleContamination = floor(iW0/2) − floor(iWF/2) + iOxF` and store it (never overwritten for oneshot→oneshot chains such as holster→draw so the idle-triggered value carries through). In `clearAnim`, subtract `_idleContamination` from the formula result; residual `K_draw` is a bounded FRM property (0 for left-edge-consistent FRMs, compensated by `K_holster` in full-swap sequences). | `src/object.ts Critter.staticAnimation, clearAnim, _idleContamination` | `animation.cc artGetFrameOffsets`; `object.cc objectSetLocation` | minor | **FIXED 2026-06-03** |
