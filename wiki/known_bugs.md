...
| FA7 | **FRM art transition caused sprite position jump (directionOffsets discontinuity).** FIXED 2026-06-03 — switching FRMs (e.g. idle `hanpwraa` with `directionOffsets[0].y=5` → weapon-draw with `directionOffsets[0].y=0`) immediately changed the renderer's `dirOffset` with no carry-over, causing a visible Y jump. CE prevents this via `_obj_offset(OLD_art->xOffsets, ...)` at animation start (`animation.cc ~2862`) which accumulates the old FRM header offsets into `obj->x/y`. DH2 fix: added `artOffset: Point` field to `Obj`; `Critter.staticAnimation()` and `Critter.clearAnim()` compute `artOffset = (old visual anchor) − (new visual anchor at frame 0)` before each art switch; renderer adds `artOffset` to the static branch (`shift === null`); artOffset resets to 0 when walking ends (equivalent to CE's `objectSetLocation` reset). | `src/object.ts Critter.staticAnimation, Critter.clearAnim`; `src/renderer.ts` | `animation.cc ~2862 _obj_offset()`; `object.cc objectSetLocation` | medium | **FIXED 2026-06-03** |
