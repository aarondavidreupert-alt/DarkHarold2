...
| FA13 | **`artOffset` formula — Attempt 5 (dirOff-only) reverted to FA12 final.** RESTORED 2026-06-06 — Attempt 5 traded zero drift for visible 2–4 px residual jumps at every weapon-swap/combat-hit/pick-up transition (`w`/`ox` mismatches between FRMs are not compensated). Live-play observation showed the per-transition jumps are more visible than the K_cycle drift they were meant to avoid, since the drift only manifests in a "stand still + 10x swap" test pathology and is reset by any walk step (CE `objectSetLocation` semantics, `object.cc:3940`). FA12 final formula restored: zero jump on every transition, drift bounded by walks. Full forensic trail: `wiki/failed_animation_offset_attempts.md` Attempts 0–5. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts objectRenderInfo, objectBoundingBox` | `animation.cc artGetRotationOffsets + artGetFrameOffsets`; `object.cc objectSetLocation` | minor | **RESTORED 2026-06-06 (FA12 final)** |
