...
| C10 | **Unarmed special moves applied in combat.** VERIFIED 2026-06-04 — `src/combat/Combat.ts` adds `mode.critBonus` to crit chance; `getUnarmedDamageDone` reads `mode.minDmg/maxDmg` for damage roll and `mode.penetrate` for the DT-to-20% bypass. Active mode selected via `getActiveUnarmedModeForHand` (player) or `getActiveUnarmedMode(skill, 0)` (AI fallback). AI hardcodes 3 AP, which matches mode 0 'punch' so no functional gap. Wiki entry was stale. | `src/combat/Combat.ts`, `src/unarmed.ts` | `unarmed.cc` | minor | fixed |
...
| W5 | **Condition system improved.** FIXED 2026-06-02 — `player(level)` now returns actual player level; `time_of_day` returns real game hour (0–23) via `getHourMilitary()/100`; `==`, `!=`, `<=`, `>=`, `or` operators added to the `op` map. CE ref: `worldmap.cc wmParseEncounterTableIndex`. | `src/encounters/conditionLang.ts` | `worldmap.cc wmParseEncounterTableIndex()` | major | partial |
...
| W9 | **Area entrance positions misplaced on area screens.** Documented in README; world map area click positions do not align with the rendered overlay markers. | `src/ui_worldmap.ts`, `src/worldmap/Worldmap.ts` | — | minor | bug |
...
| PS2 | **`tools/proto.py` sets `FO1 = True`, suppressing critter `damageType` extraction.** The `FO1` flag is a Fallout 1 compat guard; the pipeline is targeting Fallout 2 data. `damageType` is never written to the JSON, so critters always use the fallback value (0 = normal) instead of their CE-defined damage type. | `tools/proto.py:34` | `proto_types.h CritterProtoData.damageType` | major | bug |
...
| CI2 | **`game_difficulty` and `combat_difficulty` conflated.** CE has separate keys affecting skill checks/loot/XP vs enemy stats. DH2 maps both to `difficultyModifier` which only scales combat damage. | `src/config.ts:62`; `src/ui_options.ts` | `settings.h:29-31`; `preferences.cc:371-372` | minor | missing |
...
| CI9 | **`text_base_delay` added.** FIXED 2026-06-11 — `Config.ui.textBaseDelay` (default 3.5 s, range 1.0–6.0) added; slider in prefs panel (UI uses ×10 integer for precision); persisted in `SavedPreferences`. `text_line_delay` (per-line) still absent. | `src/config.ts`, `src/ui_options.ts` | `settings.h:42` | low | partial |
...
| LD5 | **`objectGetLightIntensity` self-subtraction absent.** CE subtracts the player's own `lightIntensity` from the tile value before computing effective light level (prevents self-illumination). No DH2 equivalent — moot while night-penalty is absent (lighting.md gap #1). | `src/combat/hitChance.ts` | `object.cc:1748` | low | missing |
...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| P1 | **PathFinding.js treats the hex grid as orthogonal.** CE's A* iterates all 6 hex rotations per node (`animation.cc:1795`). PathFinding.js uses 4/8-connected grid movement; hex topology mismatches may produce sub-optimal or visually odd paths near angled walls. | `src/map/GameMap.ts` | `animation.cc:1795` | minor | bug |
| P2 | **No rotation-change step cost.** CE adds +10 to a node's cost when the direction changes from its parent (outside combat). | `src/map/GameMap.ts` | `animation.cc:1838` | low | missing |
| P3 | **No radioactive goo tile penalty.** CE adds +100 (gecko) or +400 (others) to step cost when the tile contains radioactive goo PID objects. | `src/map/GameMap.ts` | `animation.cc:1852` | low | missing |
...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| EG3 | **Panning slide uses linear timing instead of CE's per-pixel formula.** CE computes `v9` (ms per pixel step) from image width and speech duration. DH2 uses a simple linear pan over `max(speechDuration, 5s)`. | `src/endgame/slideRender.ts` (`showPanningSlide`) | `endgame.cc:337-345` | low | bug |
...
| LE5 | **Ammo stack merge ignores magazine capacity.** `addInventoryItem` adds quantities without a capacity ceiling. CE fills magazines to capacity and splits remainder. | `src/object/Obj.ts` | `item.cc:322 itemAdd()` | minor | partial |
...
| LE10 | **STEALTH_BOY II auto-stealth not implemented.** CE `itemAdd` checks `PROTO_ID_STEALTH_BOY_II` and activates stealth if the item is in-hand at add time. DH2 `addInventoryItem` has no such check. | `src/object/Obj.ts` | `item.cc:322 itemAdd()` | low | missing |
...
| IW8 | **Dialogue sub-mode state machine partial.** CE `game_dialog.cc` runs a multi-state machine for dialogue/barter/trade sub-modes. DH2 only transitions `UIMode.dialogue → UIMode.barter`; other CE sub-mode paths are not replicated. | `src/ui_dialogue.ts`; `src/ui_barter/screen.ts` | `game_dialog.cc gameDialogEnter()`; `_dialogue_state` | minor | partial |
...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| AC1 | **Knockback fully implemented per CE.** FIXED 2026-06-02 — `critterDamage()` now applies knockback only for melee/unarmed/explosion damage (matching CE `combat.cc:4635` condition). Stonewall perk (defender=player): 50% chance full negation, else distance halved (CE `combat.cc:4641–4656`). CRITTER_NO_KNOCKBACK=0x4000 flag respected. Outstanding: PERK_WEAPON_KNOCKBACK divisor-5 path skipped (weapon proto perk field not loaded). | `src/critter/lifecycle.ts` | `combat.cc:4633 attackComputeDamage knockback block` | major | partial |
| AC2 | **Death animation selected by damage type.** VERIFIED 2026-06-04 — `critterKill(obj, src, useScript, animName, damageType)` resolves via priority: explicit animName → `obj.deathAnim` (set by crit effects) → `deathAnimForDamageType(dmgType)` (death-fire/plasma/laser/electro/explode) → generic 'death'. `hasAnimation` check ensures fallback when variant FRM is missing. `combat/Combat.ts` passes damageType from the killing attack. Violence-level filter still absent (CE strips gore variants based on `violence_level`); minor cosmetic. | `src/critter/lifecycle.ts`, `src/combat/Combat.ts` | `actions.cc:183 _pick_death` | minor | partial |
...
| AC6 | **First Aid / Doctor delegate to best party member.** PARTIAL 2026-06-04 — `playerUseSkill` picks the highest-skill party member for First Aid and Doctor, with a "steps in to help" log line; other skills still use the player. CE delegation rules for combat skills not modelled. CE ref: `actions.cc:1374 actionUseSkill`. | `src/playerUse.ts` | `actions.cc:1374` | minor | partial |
