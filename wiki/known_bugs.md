...
| FA13 | **`artOffset` formula — body-center / foot anchor (Attempt 6).** REFIXED 2026-06-06 — The FA12 zero-jump formula included `floor(w/2)` and `(newH − oldH)` terms thinking they were needed for visual continuity; in fact those terms anchor the **bounding-box top-left**, not the body. For hmjmps `ja→jd` dir5 (w 39→28, Δh +12) FA12 produced a measurable −5 px X / +12 px Y body snap. The correct anchor is body-centre + foot, matching CE `object.cc:2347` where `tileScreenX` is the body's horizontal centre. Final formula: `artOffset.x = oldDirOff.x − newDirOff.x + oldF.ox − newF0.ox + prev.x` and similarly for Y, **with no width or height terms**. Anchor:f0-for-idle special case from FA12 Part 1 also removed (it papered over a different bug fixed independently by FA12 Final's clearAnim formula). K_cycle drift drops from ~(−4, +11) under FA12 to ~(−1, 0) under Attempt 6 — still bounded by walk reset. Walk-end clearAnim still hard-resets to `{0,0}` (CE objectSetLocation, object.cc:3940). Full attempt history: `wiki/failed_animation_offset_attempts.md` Attempts 0–6. | `src/object.ts Critter.staticAnimation, clearAnim` | `object.cc:2347 rect anchor`; `object.cc:3940 objectSetLocation` | minor | **REFIXED 2026-06-06 (Attempt 6)** |
