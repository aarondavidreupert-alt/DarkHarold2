...
| CI6 | **`speech_volume` not persisted in DH2 preferences.** CE saves it to fallout2.cfg on every exit. | `src/ui_options/preferences.ts` | `settings.cc:93` | low | bug |
...
| ID | Description | File(s) | CE Reference | Sev | Status |
|----|-------------|---------|--------------|-----|--------|
| IW1 | **Indicator bar wired.** FIXED 2026-06-04 — `updateIndicatorBar()` injects a `#indicatorBar` element above `#bar` showing SNEAK / POISONED (`poisonLevel > 0`) / RADIATED (`radiationLevel ≥ 150`) / ADDICT (`addictions[]`) badges. Called from `drawHP` and `drawAP`. CE ref: `interface.cc indicatorBarRefresh`. | `src/ui_hud.ts:121` | `interface.cc indicatorBarInit()` | major | fixed |
