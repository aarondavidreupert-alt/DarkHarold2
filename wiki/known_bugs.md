...
| FA12 | **Per-orientation pixel drift on weapon draw.** FIXED 2026-06-03 — Root cause: `staticAnimation`'s zero-jump formula used `oldFrames[currentFrame]` (mid-idle ox, `iOxF`) as the source geometry, baking `iOxF` into `artOffset_draw`; the term then propagated through `prev` into every subsequent transition, displacing the critter by up to 9px throughout the holster animation. CE ref: `art.cc artGetFrameOffsets` — frame deltas are independent of playback position; CE never carries mid-animation ox into the object reference point. Fix: in `staticAnimation`, when `this.anim === 'idle'`, use `srcF = oldFrames[0]` (frame 0 geometry) instead of `oldFrames[currentFrame]`. The zero-jump formula becomes `floor(newF0.w/2) − floor(srcF.w/2) + srcF.ox − newF0.ox + oldDirOff − newDirOff + prev`, which is independent of which idle frame was active. A sub-pixel visual snap occurs at the trigger moment but `artOffset` is clean for the entire holster/draw chain; `K_per_cycle = 0` for well-formed FRM pairs. | `src/object.ts Critter.staticAnimation` | `art.cc artGetFrameOffsets`; `object.cc objectSetLocation` | minor | **FIXED 2026-06-03** |
