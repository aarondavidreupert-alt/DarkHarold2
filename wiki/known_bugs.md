...
| FA13 | **`artOffset` formula reduced to dirOff-only carry.** REFIXED 2026-06-06 — Iteration history: (a) FA7/FA12 full zero-jump formula gave zero per-transition jump but +4 px / cycle drift on hmjmps SMG↔laser chain (K_cycle ≠ 0). (b) Full reset on clearAnim bounded drift but caused visible snap-back. (c) Removing artOffset entirely (DarkFO direct-swap) eliminated drift but exposed −10 px X / +3 px Y snap on hmjmps kick→idle dir5 because `directionOffsets[5]` differs by 10 between `hmjmpskd` and `hmjmpsaa`. (d) Current: carry only `(oldDirOff − newDirOff)` at every transition. `w`/`ox` mismatches between FRMs remain (small residual ≤ 4 px at width-mismatched transitions), but K_cycle = 0 structurally (telescoping sum over closed cycles → zero), so no drift accumulates. Kick/jump/attack → idle settles are clean. Walk-end still hard-resets to `{0,0}` (CE objectSetLocation, object.cc:3940). Full attempt history: `wiki/failed_animation_offset_attempts.md` Attempts 0–5. | `src/object.ts Critter.staticAnimation, clearAnim`; `src/renderer.ts objectRenderInfo, objectBoundingBox` | `art.h Art.xOffsets/yOffsets`; `object.cc objectSetLocation` | minor | **REFIXED 2026-06-06** |
