...
**Status**: implemented (RD16, 2026-06-15; refined 2026-06-17; gradient restored 2026-06-23).
...
- `hilight2.png`: **Color is set in code, not in the FRM.** `HILIGHT2_COLOR` is a named constant in `tools/export_mask_frms.py`, baked into the PNG's RGB channels at conversion time — edit that constant and re-run the script to change the hue. CE's `_dark_BlendTable` is derived from `_colorTable[22187]` = `colorTable[86][171]`: the 50/50 blend of palette[86]=[120,148,120] and palette[171]=[212,172,124] ≈ [166,160,122], whose nearest palette match is **palette[119]=[160,144,124]** — a warm tan-gray. Pixels in the darkened region get shifted toward that color in CE's blend math. **Corrected 2026-06-24**: `HILIGHT2_COLOR = (160, 144, 124)` replacing the earlier wrong amber `(255,140,30)`. The amber impression came from the alltlk.png backdrop's own warm tones visible through the shadow, not from the blend color. See `wiki/palette_colors.md §3.1` for the full derivation. `HIGHLIGHT_STRENGTH` (= 1.0) and `HILIGHT2_COLOR` are named constants for easy re-tuning.
