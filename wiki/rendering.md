...
**DH2 implementation (2026-06-23)**:

1. **Asset fix first**: the original concern above — that a literal `mix-blend-mode` on the raw PNGs wouldn't work because their pixel values are palette-mangled blend-weights, not colors — was correct for the *originally pipeline-converted* PNGs. Fixed at the source instead of working around it: a new standalone, pipeline-isolated script `tools/export_mask_frms.py` (does **not** touch `tools/frmpixels.py`/`tools/exportImagesPar.py` — re-running the full asset pipeline for 3 files is unnecessary) parses each FRM directly and writes its raw mask byte into the **alpha channel** of a solid-white RGBA PNG, skipping the palette lookup entirely. Regenerated `hilight1.png`/`hilight2.png` (and `egg.png`, see "Egg Transparency Effect" above) this way; pixel inspection afterward confirmed real gradients (e.g. `hilight1.png` alpha range 0-254) instead of the garish palette-index colors the normal conversion produced.
2. **Rendering**: rather than the per-pixel canvas blend originally planned, `#dialogueHighlightUpper`/`#dialogueHighlightLower` are now permanent `<img>` children of `#dialogueContainer` (`play.html`) at the verified positions (426,15 / 129,81), with CSS `mix-blend-mode: screen` (upper, lightening) and `mix-blend-mode: multiply` (lower, darkening) (`ui.css`). Since the PNGs now carry a genuine alpha gradient, the browser's native alpha-weighted blend compositing reproduces the same lighten/darken-by-weight effect CE's gray-table math does, with far less code, and it automatically composites correctly against whatever ends up in the head rect later (no need to manually recapture the background into a canvas first).
3. **Not addressed**: the talking-head/game-view content gap from earlier in this section is still open — DH2's head rect is still the static `alltlk.png`, so the highlight currently blends against that rather than an animated head or live game-view crop. The highlight is still a real visual improvement on its own, but full fidelity depends on that separate, larger gap eventually getting implemented.
4. **Known simplification**: CSS blend-mode compositing approximates CE's exact per-palette-index gray-table math; it is not a pixel-identical port, but the visual character (soft glint upper-right, shadow lower-left) matches.

See `wiki/known_bugs.md` P22 for the full change list and citations.
