...
**Roof clipping** (RD06, FIXED 2026-06-15): `renderer.ts` flood-fills (`roofFloodFill`) the connected roof section the player stands under from their square tile each frame (matching CE `tile_fill_roof`), passing a `hideSet` to `renderRoof()` which skips those tiles. `Config.ui.showRoof` (default `true`) is a separate global on/off. Extended 2026-07-02 (`Config.ui.roofPeek`, default on): because roofs draw 96px up, the roof covering the player when they stand *behind* a building is ~2-3 squares south, so DH2 also floods from the roof tile at the player's screen position + 96px and unions the hide-set, revealing the character behind buildings (CE only floods from the player's own square). See **RD06** in §5.
...
| RD06 | Roof clipping | `tile_fill_roof` flood-fills connected roof tiles from the player's square when under a building; re-evaluated each frame (`object.cc:1445`) | `roofFloodFill` from the player's square → `hideSet` → `renderRoof` skips them; **plus** `roofPeek`: also floods from the roof tile covering the player (screen + 96px) so behind-building roofs reveal the character | — | ✅ Fixed 2026-06-15; `roofPeek` extension 2026-07-02 (`known_bugs.md` RD06) |
