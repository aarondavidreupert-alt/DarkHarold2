...
- **Open issue, not yet root-caused**: walls immediately adjacent to the player sometimes show no fade at all even though they pass the `isEggObject` qualification (hex angle + distance gate), while walls 2-3 tiles away show the full oval clearly. The qualification gate (CE-ported `hexIsInFrontOf`/`hexIsToRightOf`, verified against `tile.cc:854/871`) is purely geometric and doesn't account for whether the sampled point inside the small egg footprint actually falls in the oval vs. the transparent bounding-box corner — but this hasn't been confirmed as the actual cause, and it's unconfirmed whether the original game has this same gap (do not assume CE-parity here without checking real footage). See `wiki/known_bugs.md` RD16.
