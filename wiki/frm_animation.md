...
| FA6 | **FID composition / weapon stance animation not implemented.** CE builds a Frame Identifier via `buildFid(objectType, animType, weaponAnimCode, direction, rotation)` (`art.cc`), where `weaponAnimCode` (from the weapon proto's `weaponAnimCode` field) selects the critter's armed-pose FRM set (0=unarmed, 1=pistol, 3=rifle, 4=big gun, etc.). When a critter equips a weapon their animation FRM switches to the matching armed-stance sprites. DH2 has no `buildFid` equivalent — critter FRM paths are resolved from static skin strings (`getSkin()`), never modified by the held weapon. Critters always display their unarmed walk/idle animations regardless of what weapon they hold. The weapon art appears only in the HUD via `uiDrawWeapon()`. | `src/object.ts`; `src/renderer.ts` | `art.cc buildFid()`; `art.h ART_TYPE_CRITTER`; `proto_types.h ItemWeaponData.animCode` | medium | missing |
