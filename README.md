...
| 43 | [`wiki/alignment.md`](wiki/alignment.md) | Screen-space anchor reference for all renderable categories (floor/roof `−96`, object bottom-centre anchor, `uniformFrameWidth` vs `frameWidth`, `8.4` lightmap-UV inverse) **plus the three lighting-alignment fixes** — light centring offset (RD17 §6), interpolation stripes / `hex-lerp` (§7), and W-E wall occlusion bleed (LD11 §8), all rooted in `hexToScreen`'s per-column parity |
...
- **Map loading & rendering** — tile maps, multi-elevation, WebGL 2.0 renderer, lightmap, real-time lighting, screen-space hex z-sort (RD09), camera clamp + `OBJECT_SCROLL_BLOCK` (RD11/RD12), per-building roof flood-fill (RD06), parity-correct lightmap hex sampling with selectable interpolation (RD17, default `hex-lerp`), directional wall light occlusion via `extendedFlags` (LD11) — see [`wiki/alignment.md`](wiki/alignment.md)
...
- **Lighting** — `obj_set_light_level` + `set_obj_visibility` correctly rebuild the lightmap (LD3/LD4), hidden objects no longer emit light (LD1), directional wall occlusion reads `extendedFlags` so W-E walls no longer bleed light (LD11). Day/night ambient curve is a DH2 invention rather than CE-matched (GTC10); `objectGetLightIntensity` self-subtraction absent (LD5); non-wall opaque-object shadowing still stubbed (LD11 note).
