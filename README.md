...
- **Map loading & rendering** — tile maps, multi-elevation, WebGL 2.0 renderer, lightmap, real-time lighting, screen-space hex z-sort (RD09), camera clamp + `OBJECT_SCROLL_BLOCK` (RD11/RD12), per-building roof flood-fill (RD06), parity-correct lightmap hex sampling with selectable interpolation (RD17, default `hex-lerp`), directional wall light occlusion via `extendedFlags` (LD11), per-column wall light sampling (`wall-clamp` default) with alpha-silhouette top-edge fade, and smooth moving-torch lighting (`egg-split`) — see [`wiki/alignment.md`](wiki/alignment.md)
