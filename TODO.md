...
| P2 | `tile_contains_pid_obj` | Partial — logic exists and runs but stub() still logs; verify correctness |
...
| P3 | `critter_attempt_placement` | Partial — calls move_to() but doesn't search adjacent tiles when occupied |
...
- **[P2] `damage_p_proc` call** (`critter.ts:567`)
  - `Scripting.damage()` exists but is never invoked on damage; critter scripts' damage procedure never fires
...
- **[P2]** `get_month` and `get_day` opcodes — verify these are correctly wired in scripting.ts; previously hardcoded to return 1 / 0
