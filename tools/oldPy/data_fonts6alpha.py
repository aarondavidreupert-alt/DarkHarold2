# -*- coding: utf-8 -*-
"""
Created on Sun Apr 19 22:48:29 2026
...
# AAF pixels are 0–9 (blend weight, like fallout-CE's color blend table).
# Color is always full intensity — AAF value controls alpha (transparency).
# This matches fallout-CE: palette[(byte << 8) + bg] — byte is the blend weight.
AAF_ALPHA = [0, 30, 66, 116, 116, 169, 219, 255, 255, 255]
        ...
        if s['width'] > 0 and s['height'] > 0:
            ...
            for py in range(s['height']):
                for px_ in range(s['width']):
                    ...
                    if v > 0:
                        pix[px_, py] = (
                            color[0],      # full R — always target color
                            color[1],      # full G
                            color[2],      # full B
                            AAF_ALPHA[v]   # alpha = blend weight (0=transparent, 255=opaque)
                        )
