...
# AAF_FULL_INTENSITY = True  → divide by 9.0  (correct, full range)
# AAF_FULL_INTENSITY = False → divide by 7.0  (original behaviour, slightly brighter)
AAF_FULL_INTENSITY = True
        ...
        if s['width'] > 0 and s['height'] > 0:
            ...
            for py in range(s['height']):
                for px_ in range(s['width']):
                    ...
                    if v > 0:
                        intensity = v / 9.0 if AAF_FULL_INTENSITY else v / 7.0
                        pix[px_, py] = (
