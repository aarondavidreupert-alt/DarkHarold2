# -*- coding: utf-8 -*-
"""
Created on Sun Apr 19 22:31:47 2026
...
# AAF pixels are 0–9 (brightness scale).
# AAF_GAMMA controls brightness curve:
#   "linear"  → v/9.0          (mathematically correct, but looks dim)
#   "gamma"   → (v/9.0)**0.6   (gamma corrected, perceptually balanced) ← recommended
#   "legacy"  → v/7.0          (original behaviour, too bright/clipped)
AAF_GAMMA = "gamma"   # change to "linear" or "legacy" to compare
...
def parse_aaf(data, color=(255, 200, 0)):
    ...
    for i in range(256):
        off = 12 + i * 8
        w, h = struct.unpack('>2H', data[off:off+4])
        doff = struct.unpack('>I', data[off+4:off+8])[0]  # big-endian (Motorola) per spec
    ...
    for i in range(256):
        ...
        if s['width'] > 0 and s['height'] > 0:
            ...
            px_data = data[start:start + expected]
            if len(px_data) < expected:
                cur_x += s['width']
                continue
...
for i in range(6):
    for ext, parser in [('aaf', parse_aaf), ('fon', parse_fon)]:
        ...
        try:
            ...
            print(f"✗ font{i}.{ext} — {e}")h.join(out_dir, f'font{i}_{ext}.png')
            json_out = os.path.join(out_dir, f'font{i}_{ext}.json')
            img.save(png_out)
            with open(json_out, 'w') as f:
                json.dump({str(k): v for k, v in si.items()}, f)
            ...
            print(f"✗ font{i}.{ext} — {e}")# -*- coding: utf-8 -*-
"""
Created on Wed Apr 15 22:46:34 2026
