...
| W10 | ✅ FIXED 2026-07-04 — `.msk` files are fetched directly at runtime (no pipeline changes needed — `data/*.msk` ships as raw binary in the DAT archives, same as `worldmap.txt`) and checked before each worldmap movement step; travel halts in place at the boundary instead of walking through. See Phase 10e for the full writeup. | `worldmap.cc:1337 wmGrabTileWalkMask()` | minor |
