...
| EL4 | ✅ Investigated 2026-07-05 — not a real gap; the bitmask is purely a `.map`-file-stream-skip detail, and DH2's `levels` array length already losslessly encodes the same elevation-presence information. See Phase 10 row below for the full writeup. | `map.cc:81` | low |
...
| K3 | ✅ FIXED 2026-07-05 — real interactive stealing UI added (`src/ui_steal.ts`), replacing the old single-abstract-roll `useSteal()`. Reuses the loot window's drag-and-drop chrome against a living critter's inventory; every drag rolls `performSteal()` (`src/skillUse.ts`) per item — real `-4×itemGetSize` penalty (`item.pro.extra.size`), facing (-25) and knocked-down (+20) modifiers, session `stealCount` penalty (CE's `_gStealCount`), and the real double-roll mechanic (stealRoll's crit thresholds gate a second independent catchRoll — the well-known FO2 quirk where skilldex Steal% isn't the true odds). Getting caught ends the session and fires the target's `pickup_p_proc`. Bonus XP (`+10`/success, capped `300-skill`) awarded once at session end, matching `inventory.cc:4368-4470`. Dead/knocked-out targets fall back to a free `uiLoot()` (matches CE's `critterIsActive` gate on `_gIsSteal`). | `skill.cc:1031-1110`, `inventory.cc:4505-4660` | minor | fixed |
...
| LE10 | ✅ FIXED 2026-07-05 — full charge/toggle system added (new `src/miscItem.ts`). Pipeline gap closed first: `tools/proto.py` never parsed misc-item `data.misc` (`powerTypePid`/`powerType`/`charges`) at all — added the `SUBTYPE_MISC` branch (`ProtoItemMiscData`, 3×int32), new `MiscItemProtoExtra` in `proto_types.ts` (real pipeline re-run against actual assets still needed locally to regenerate `proto/items/*.json`). Toggling via `Obj.use()` (the "Use" inventory action) calls `useChargedMiscItem()`: turns on/off, drains 1 charge per in-game minute via `Scripting.timeEventList` (matching CE's 600-tick trickle), auto-reactivates on re-equip into a hand slot (the literal "auto-stealth" ask) via a new `Critter.stealthActive` flag recomputed by `refreshStealthState()` on every equip/unequip/swap/drop. `Combat.ts`'s `findTarget()` now halves max AI detection range for a stealthed critter, matching `isWithinPerception()`. State (`miscOn`/`miscCharges`) is serialized on the item; **known simplification**: DH2's save/load re-equip pass only restores weapon-subtype items into hand slots, so a Stealth Boy equipped at save time falls back to inventory on load (charge state still round-trips; just needs manual re-equip). **Not implemented**: CE's `OBJECT_TRANS_GLASS` semi-transparent sprite rendering — needs a real WebGL shader change verified in a browser, deferred to the local/visual session (same as RD10 color-cycling). | `item.cc:2246-2499`; `combat_ai.cc:3499-3520`; `proto_types.h ProtoItemMiscData` | low | fixed |
| EL4 | ✅ Investigated 2026-07-05, not a real gap — CE's `_map_data_elev_flags` (`map.cc:81`, `[2,4,8]`) exists purely so `_square_load`/`mapSave` (`map.cc:1377-1385,1404,1714`) know which elevations' tile blocks to skip in the binary `.map` file stream; it's a *file-format* detail, not a distinct runtime concept. `tools/fomap.py`'s `getNumLevels(elevationFlags)` (line 48) already replicates the exact same bit logic to read only the real elevations out of the stream, and the output JSON's `levels` array is simply as long as the map actually has elevations — no separate bitmask field is needed since the array length *is* the presence data, losslessly. Confirmed every DH2 runtime consumer (`GameMap.ts:changeElevation` bounds-checks `mapObj.levels[level]`; `mapLoader.ts:193` derives `numLevels = map.levels.length`; `input.ts`, `automap/tracking.ts` all iterate dynamically) — none hardcode 3 elevations. The one theoretical gap (a non-contiguous pattern like elevation 0 absent but 1 present) isn't representable by DH2's array-length model, but no known FO2 map uses that pattern — every real map's elevations start at 0 and go up contiguously, matching CE's own mapper convention. | `map.cc:81,1377-1385,1404,1714`; `tools/fomap.py:48` | low | not-a-gap |
